...
with strategy.scope():
    ...
    x = layers.Conv2D(64, 3, activation="sigmoid", strides=(1, 2), padding="same")(encoder_inputs)
    x = layers.Conv2D(32, 3, activation="sigmoid", strides=(1, 4), padding="same")(x)
    x = layers.Conv2D(16, 3, activation="sigmoid", strides=(2, 3), padding="same")(x)

    x = layers.Flatten()(x)
    x = layers.Dense(286, activation="sigmoid")(x)
    ...
    x = layers.Dense(286, activation="sigmoid")(latent_inputs)
    x = BatchNormalization()(x)
    x = layers.Dense(1472, activation="sigmoid")(x)
    x = layers.Reshape((23, 4, 16))(x)
    x = layers.Conv2DTranspose(16, 3, activation="sigmoid", strides=(2, 3), padding="same")(x)
    x = layers.Conv2DTranspose(32, 3, activation="sigmoid", strides=(1, 4), padding="same")(x)
    x = layers.Conv2DTranspose(64, 3, activation="sigmoid", strides=(1, 2), padding="same")(x)
