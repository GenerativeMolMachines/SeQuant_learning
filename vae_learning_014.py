...
from vae_012 import autoencoder_model
...
# Data preprocessing
train_dataset = data_processing(batch_data=train_data, monomer_dict=monomer_dict, max_len=max_len)
test_dataset = data_processing(batch_data=test_data, monomer_dict=monomer_dict, max_len=max_len)
...
for strategy in filter_strategy:
    ...
    # set checkpoint
    checkpoint_filepath = f'checkpoint/checkpoint_vae_014'
    ...
    with open(f'trainHistoryDict/vae_014.pkl', 'wb') as file_pi:
        pickle.dump(history.history, file_pi)

    # load model learning history
    with open(f'trainHistoryDict/vae_014.pkl', 'rb') as f:
