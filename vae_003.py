...
def vae_loss(original_inputs, outputs, mu, log_var):
    ...
    kde_q = tfp.distributions.MultivariateNormalDiag(loc=tf.reduce_mean(z_q, axis=0),
                                                     scale_diag=tf.math.reduce_std(z_q, axis=0))
    kde_p = tfp.distributions.MultivariateNormalDiag(loc=tf.reduce_mean(z_p, axis=0),
                                                     scale_diag=tf.math.reduce_std(z_p, axis=0))
    p_q = kde_q.prob(z_q)
    p_p = kde_p.prob(z_p)

    m = 0.5 * (p_q + p_p)
    kl_pm = tf.reduce_mean(p_q * tf.math.log(p_q / (m + 1e-10)))
    kl_qm = tf.reduce_mean(p_p * tf.math.log(p_p / (m + 1e-10)))
    jsd = 0.5 * (kl_pm + kl_qm)

    return tf.reduce_mean(reconstruction_loss + jsd)
